...
public class MainFrame extends JFrame {
	...
	private MainFrame() {
		...
		setLayout(gbl);
		setBounds((screenWidth - width) / 2, (screenHeight - height) / 2,
				width, height);
	...
	public static void main(String[] args) {
		...
		// 欢迎动画
		/*
		 * InitialThread thread=new InitialThread(); thread.startThread();
		 */
		/*
		 * final AnimeFrame p=new AnimeFrame(); final boolean stop=false; final
		 * InitialThread thread=new InitialThread(p); thread.startThread();
		 * 
		 * t1=new Timer();t2=new Timer(); t1.schedule(new TimerTask(){
		 * 
		 * @Override public void run() { p.setBackImage(new
		 * ImageIcon("image/back.png")); t2.schedule(new TimerTask(){ public
		 * void run(){
		 * 
		 * while(p.stop){
		 * 
		 * p.dispose(); MainFrame.getInstance().setVisible(true);
		 * //t1.cancel();t2.cancel(); IndexPanel.startT(); break;
		 * 
		 * 
		 * } } }, 1100);
		 * 
