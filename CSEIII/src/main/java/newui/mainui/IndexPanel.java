...
public class IndexPanel extends FatherPanel implements MouseListener {
	...
	public IndexPanel() {
		...
		TeamBLService team2 = Service.team;
		team2.getClass();
		...
		if (!UIhelper.isImgExists(pImgName))
			pImgName = "null";
		ImageIcon progressIcon = new ImageIcon("image/player/action/"
				+ pImgName + ".png");
		progressIcon.setImage(progressIcon.getImage().getScaledInstance(220,
	...
	public void Refresh() {
		...
		if (!sname.equals(s)) {
			...
			seasonIcon.setImage(seasonIcon.getImage().getScaledInstance(220,
					350, Image.SCALE_SMOOTH));
			seasonPnl.iconLbl.setIcon(seasonIcon);
		}
		if (!tname.equals(tn)) {
			...
			tPlayername = tt;
			teamPnl.setTitleAndName("场均得分最高球队", tname);
			ImageIcon teamIcon = new ImageIcon("image/player/action/"
					+ tPlayername + ".png");
			teamIcon.setImage(teamIcon.getImage().getScaledInstance(220, 350,
					Image.SCALE_SMOOTH));
			teamPnl.iconLbl.setIcon(teamIcon);
		...
		if (!pname.equals(p)) {
			...
			ImageIcon progressIcon = new ImageIcon("image/player/action/"
					+ pImgName + ".png");
			progressIcon.setImage(progressIcon.getImage().getScaledInstance(
					220, 350, Image.SCALE_SMOOTH));
			progressPnl.iconLbl.setIcon(progressIcon);
		}
		this.repaint();
