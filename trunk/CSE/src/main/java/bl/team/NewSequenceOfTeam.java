...
public class NewSequenceOfTeam implements Comparator<TeamVO> {
	private String column;
	private String order;
	private ArrayList<String> allColumn;
	private ArrayList<String> allOrder;

	public NewSequenceOfTeam(ArrayList<String> column, ArrayList<String> order) {
	...
	public int compare(TeamVO t1, TeamVO t2) {
		...
		for(int i=0;i<allColumn.size();i++){
			result=compare(t1, t2, allColumn.get(i), allOrder.get(i));
			if(result!=0){
				break;
			}
		}
	...
	private int compare(TeamVO t1, TeamVO t2,String c,String o) {
		// TODO 自动生成的方法存根
		int result = 0;
        column=c;
        order=o;
