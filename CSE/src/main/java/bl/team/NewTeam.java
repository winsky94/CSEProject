...
//			 System.out.println("matchesNum："+vo.getMatchesNum());
	...
	public ArrayList<TeamVO> getTeamSeasonInfo(String season, String name) {
		...
		while (iter.hasNext()) {
			...
			boolean flag = false;
			if (name.equals("NOP") || name.equals("NOH")) {
				temp = teamsBaseInfo.get("NOP").getTeamName();
				if (temp == null) {
					temp = teamsBaseInfo.get("NOH").getTeamName();
				}
			}
			name = name.toLowerCase();
			flag = teamName.toLowerCase().contains(temp)
					|| team.toLowerCase().contains(temp);
			if (!flag) {
	...
	public ArrayList<TeamVO> getOrderedTeamsByAverage(
			ArrayList<String> condition, ArrayList<String> order, int num) {
		// TODO 自动生成的方法存根
		if (num < 0) {
			num = 30;
